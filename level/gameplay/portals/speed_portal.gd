...
func _ready() -> void:
	...
	if speed_level >= 0:
		sprite.region_rect.size.x = 12 * (speed_level + 1)
		sprite.scale.x = min(1.0, 1.5 / (1 + 0.2 * (speed_level + 1)))
	else:
		sprite.region_rect.size.x = 12 * absf(speed_level)
	...
	# original region rect width was 12
	var sprite_scaled_by := (sprite.region_rect.size.x / 12.0) * absf(sprite.scale.x)
	if new_shape:
